 ...
 */
class Resourceapi extends ModuleObject
{
	/**
...
class ObjectAndXML {
	private static $xml;
	private $srls = array('srl','error','message','depth','package_vote','package_downloaded','item_vote','item_downloaded','package_star');
	  ...
	  private function getObject2XML(XMLWriter $xml, $data) {
		  foreach($data as $key => $value) {
			  ...
			  if (is_string($value) || is_numeric($value)) {
				  $xml->startElement($key);
				  if(str_replace($this->srls, '', $key) != $key)
						$xml->text($value);
					else
